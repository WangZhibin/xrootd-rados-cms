...
public:
  ...
  /**
   * @brief return the default pool name taken from rados
   * @return 
   */
  std::string GetDefaultPoolName (void) const;

  /**
   * @brief initialize all defined pool IoCtx in the pool map
   * @return nothing
   */
  void InitIoctxInPools ();

  /**
   * @brief select the appropriate IoCtx for the given object path
   * 
   * @param objectName path to the object
   * @param ioctx to be returned
   * @return 0 if success
   */
  int GetIoctxFromPath (const std::string &objectName,
                        rados_ioctx_t *ioctx);

  /**
   * @brief return a corresponding pool for a given path
   * 
   * @param path
   * @return pool to use for given path
   */
  const RadosCmsPool* GetPoolFromPath(const std::string &path);
...
private:
  ...
  rados_t mCephCluster; ///< ceph cluster variable
  std::string mCephUser; ///< user name used to connect
  std::string mCephConf; ///< ceph conf file to be used
